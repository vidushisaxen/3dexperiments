...
const useRouteChange = (delay = 0.5, duration = 1) => {
  ...
  const routeColors = {
    '/': { baseBlue: [0.1, 0.3, 0.7], baseYellowGreen: [0.5, 0.8, 0.3] },
    '/services': { baseBlue: [0.2, 0.4, 0.9], baseYellowGreen: [0.7, 0.4, 0.2] },
    '/contact': { baseBlue: [0.5, 0.1, 0.2], baseYellowGreen: [0.2, 0.9, 0.6] },
    '/about': { baseBlue: [0.0, 0.1, 0.2], baseYellowGreen: [0.2, 0.5, 0.6] },
    '/works': { baseBlue: [0.5, 0.0, 0.6], baseYellowGreen: [0.8, 0.8, 0.0] },
    '/portfolio': { baseBlue: [0.0, 0.8, 0.5], baseYellowGreen: [0.6, 0.8, 0.9] }
  };

  const getInitialColors = () => {
    return routeColors[router.pathname] || {
      baseBlue: [0.1, 0.1, 0.1],
      baseYellowGreen: [0.2, 0.2, 0.2]
    };
  };
  ...
  useEffect(() => {
    if (document.readyState === 'complete') {
      const targetColors = routeColors[router.pathname] || {
        baseBlue: [0.1, 0.1, 0.1],
        baseYellowGreen: [0.2, 0.2, 0.2]
      };
      ...
      const colorAnimation = setTimeout(() => {
        gsap.to(proxy, {
